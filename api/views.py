...
from products.models import BoxProduct, Customer, Flavor, Order, Product, ShippingAddress
    ...
    if serializer.is_valid():
      ...
      if (category_code == '2' and ('box_product_id' in request.data or 'orderFlavor' in request.data)):
        ...
        try:
          ...
          instance_box_product = BoxProduct.objects.get(id=box_product_id)
  ...
  def delete(self, request, pk, *args, **kwargs):
    ...
    if category_code == '2':
      try:
        ...
        print('yep box product count: ', order_item.box_product.count())
...
class ProcessOrderViewSet(viewsets.ViewSet):
  def create(self, request, *args, **kwargs):
    ...
    if total == float(order.get_cart_total) or not request.user.is_authenticated:
...
class DatePicker(viewsets.ViewSet):
  def list(self, request, *args, **kwargs):
    ...
    if request.user.is_authenticated:
      ...
      total_items_calendar = sum(item['quantity'] for item in cookies_cart)
